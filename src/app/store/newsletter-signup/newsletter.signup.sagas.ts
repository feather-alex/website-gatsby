...
export function* handleNewsletterSignup(action: FluxStandardAction): SagaIterator {
  try {
    const email = action.payload.requestData.email;
    const origin = action.payload.requestData.origin;

    yield call([Request, 'send'], RequestMethod.POST, '/signup', undefined, { email }, true);
    ...
    yield call(Analytics.trackEvent, NEWSLETTER.SIGNUP, signupPayloadMapping({ email, origin }));
