...
describe("Overlay Selectors", () => {
  ...
  it("should return whether the mini cart is open", () => {
    expect(
      getIsMiniCartOpen({ app: { overlay: state } } as GlobalState)
    ).toEqual(false);
  });

  it("should return whether the mobile product filter is open", () => {
    expect(
      getIsMobileProductFiltersOpen({ app: { overlay: state } } as GlobalState)
    ).toEqual(true);
  });

  it("should return whether the quiz overlay is open", () => {
    expect(
      getIsQuizOverlayOpen({ app: { overlay: state } } as GlobalState)
    ).toEqual(false);
  });

  it("should return whether the product info overlay is open", () => {
    expect(
      getIsProductInfoOverlayOpen({ app: { overlay: state } } as GlobalState)
    ).toEqual(false);
  });

  it("should return whether the dimensions overlay is open", () => {
    expect(
      getIsDimensionsOverlayOpen({ app: { overlay: state } } as GlobalState)
    ).toEqual(false);
  });

  it("should return whether the delivery overlay is open", () => {
    expect(
      getIsDeliveryOverlayOpen({ app: { overlay: state } } as GlobalState)
    ).toEqual(true);
  });
  ...
  it("should return whether the failed ssn overlay is open", () => {
    expect(
      getIsFailedSSNOverlayOpen({ app: { overlay: state } } as GlobalState)
    ).toEqual(false);
  });

  it("should return whether the no ssn overlay is open", () => {
    expect(
      getIsNoSSNOverlayOpen({ app: { overlay: state } } as GlobalState)
    ).toEqual(false);
  });

  it("should return whether the no ssn overlay is open", () => {
    expect(
      getIs3DOverlayOpen({ app: { overlay: state } } as GlobalState)
    ).toEqual(false);
  });

  it("should return the whole overlay state", () => {
    expect(getAllOverlays({ app: { overlay: state } } as GlobalState)).toEqual(
      state
    );
  });
  ...
  describe("Getting the currently open overlay", () => {
    it("should return the currently open overlay when one is open", () => {
      ...
      expect(
        getOpenOverlay({ app: { overlay: overlayState } } as GlobalState)
      ).toEqual(Overlays.DimensionsOverlay);
    });
