...
describe("Navbar Selectors", () => {
  ...
  it("should return whether the navbar banner is visible", () => {
    expect(
      getIsNavbarBannerVisible({ app: { navbar: state } } as GlobalState)
    ).toEqual(true);
  });

  it("should return the current type of navbar banner", () => {
    expect(
      getNavbarBannerType({ app: { navbar: state } } as GlobalState)
    ).toEqual(BannerType.Announcement);
  });

  it("should return the current navbar banner message", () => {
    expect(
      getNavbarBannerMessage({ app: { navbar: state } } as GlobalState)
    ).toEqual("Select Banner Message");
  });

  it("should return the current navbar banner color", () => {
    expect(
      getNavbarBannerColor({ app: { navbar: state } } as GlobalState)
    ).toEqual("#00ff00");
  });

  it("should return the current navbar content", () => {
    expect(getNavbarContent({ app: { navbar: state } } as GlobalState)).toEqual(
      null
    );
  });
  ...
  it("should return if the content is currently loading", () => {
    expect(
      getNavbarIsContentLoading({ app: { navbar: state } } as GlobalState)
    ).toEqual(false);
  });
});
