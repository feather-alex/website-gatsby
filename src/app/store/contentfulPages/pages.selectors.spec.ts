...
describe("Contentful Pages - Selectors", () => {
  ...
  it("Should return a list of: Contentful pages", () => {
    ...
    expect(
      getPages({ app: { contentfulPages: state } } as GlobalState)
    ).toEqual(mockSuccessPayload.pages);
    expect(
      getPages({ app: { contentfulPages: state } } as GlobalState).length
    ).toEqual(2);
  });
  ...
  it("Should return a list of: City pages", () => {
    ...
    expect(
      getCityPages({ app: { contentfulPages: state } } as GlobalState)
    ).toEqual([mockSuccessPayload.pages[0]]);
    expect(
      getCityPages({ app: { contentfulPages: state } } as GlobalState).length
    ).toEqual(1);
  });
  ...
  it("Should return the value of: isFetching", () => {
    ...
    expect(
      getIsFetchingPages({ app: { contentfulPages: state } } as GlobalState)
    ).toBeTruthy();
  });
  ...
  it("Should return the value of: error", () => {
    ...
    expect(
      getPagesError({ app: { contentfulPages: state } } as GlobalState)
    ).toEqual(mockError);
  });
});
