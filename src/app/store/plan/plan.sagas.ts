...
export function* handleZipcode(action: FluxStandardAction): SagaIterator {
  try {
    ...
    const response: any = yield call([Request, 'send'], RequestMethod.GET, `/delivery/postal/${enteredZipcode}`);

    const isPlanSelectionOverlayOpen = yield select(getIsPlanSelectionOverlayOpen);
    const isNavbarBannerOpen = yield select(getIsNavbarBannerVisible);
    const navbarBannerType = yield select(getNavbarBannerType);
    const { pathname } = yield select((state: GlobalState) => state.router.location);
...
export function* handleMembershipChange({ payload: { membershipState } }: FluxStandardAction): SagaIterator {
  const isPlanSelectionOverlayOpen = yield select(getIsPlanSelectionOverlayOpen);
  ...
  yield call(Analytics.trackEvent, membershipChangeEventMap[membershipState], {
    origin: determinePlanActionOrigin({ isPlanSelectionOverlayOpen, isNavbarBannerOpen, navbarBannerType, pathname })
  });
  ...
  if (membershipState !== MembershipState.NONE && isInDeliveryZone) {
    const message = `You’ve selected ${
      membershipState === MembershipState.MEMBER ? 'Membership' : 'Short-Term Plan'
