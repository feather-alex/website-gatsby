...
const BackInStockInfo = ({ deliveryAreaIdentifier, availability }: Props) => {
  if (!isInStock(deliveryAreaIdentifier, availability) && backInStockDate(deliveryAreaIdentifier, availability)) {
    ...
    if (dateFromApi) {
      ...
      return (
        <BackInStockContainer>
          <CalendarIcon />
          <BackInStockTitle isBold={true}>Expected back in stock on {formattedStockDate}</BackInStockTitle>
        </BackInStockContainer>
      );
    }
  }
