...
describe('<ProductPairings />', () => {
  it('should call to fetch product pairings on mount', () => {
    ...
    renderWithRedux(
      <ConnectedRouter history={history}>
        <ProductPairings productIdentifiers={productIdentifiers} categoryIdentifier={categoryIdentifier} />
      </ConnectedRouter>,
      undefined,
      store
    );

    expect(mockDispatch).toHaveBeenCalledWith(getProductPairingsRequest(expectedProductPairingsRequestPayload));
  });
  ...
  it('renders a maximum of four product cards despite how many product pairing are provided', () => {
    const { store, history } = createMockStoreAndHistoryWithState({
      ...fullInitialState,
      productPairings: { ...initialState, products: [...mockItems, makeMockProductForListing(5)] }
    });
    store.dispatch = jest.fn();
