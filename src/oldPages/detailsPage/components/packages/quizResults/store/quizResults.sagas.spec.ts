...
import { handleQuizResultsRequest, handleQuizResultsRequestByUuid, handleUpdateQuizResults } from './quizResults.sagas';
...
describe('Quiz Results - Sagas', () => {
    ...
    it('should handle successfully fetching quiz results', () => {
      return expectSaga(handleQuizResultsRequest)
        .provide([
          ...
          [matchers.call([Request, 'send'], RequestMethod.POST, '/quiz/results', undefined, quizAnswers), quizPkgs]
        ])
    ...
    it('should handle unsuccessfully fetching quiz results', () => {
      return expectSaga(handleQuizResultsRequest)
        .provide([
          [matchers.select(getQuizAnswers), quizAnswers],
          [
            matchers.call([Request, 'send'], RequestMethod.POST, '/quiz/results', undefined, quizAnswers),
  ...
  describe('Fetch quiz results by uuid', () => {
    it('should handle successfully fetching quiz results', () => {
      ...
      return expectSaga(handleQuizResultsRequestByUuid, action)
        .provide([
          [matchers.call([Request, 'send'], RequestMethod.GET, `/quiz/results/${uuid}`, undefined), quizPackages]
        ])
        .put(
          fetchQuizResultsSuccess({
            ...quizPackages.items,
    ...
    it('should handle unsuccessfully fetching quiz results', () => {
      ...
      return expectSaga(handleQuizResultsRequestByUuid, action)
        .provide([
          [
            matchers.call([Request, 'send'], RequestMethod.GET, `/quiz/results/${uuid}`, undefined),
            Promise.reject(error)
          ]
        ])
        .put(fetchQuizResultsFailure(error))
  ...
  describe('Update quiz results', () => {
    it('should handle successfully updating quiz results', () => {
      const action: QuizResultsActions = updateQuizResultsRequest(uuid, quizPackages);

      return expectSaga(handleUpdateQuizResults, action)
        .provide([
          [
            matchers.call([Request, 'send'], RequestMethod.PUT, `/quiz/results/${uuid}`, undefined, quizPackages),
            quizPkgs
          ]
        ])
