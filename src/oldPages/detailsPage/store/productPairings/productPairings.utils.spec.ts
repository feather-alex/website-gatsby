...
describe('Utility functions for product pairings', () => {
  describe('filterPariedProducts', () => {
    it('should filter out origin products from the product pairings', () => {
      ...
      const result = filterPairedProducts(mockProductPairings, mockPairedProductIdentifiers);

      expect(result).toEqual([{ identifier: 'crosby-desk-chair' }, { identifier: 'varick-console' }]);
    });
