...
export const isFetching = ({ accounts }: GlobalState) => accounts.accountOverview.isFetching;
export const getError = ({ accounts }: GlobalState) => accounts.accountOverview.error;
export const getItems = ({ accounts }: GlobalState) => accounts.accountOverview.overview.items;
export const getMonthlySubtotal = ({ accounts }: GlobalState) => accounts.accountOverview.overview.monthlySubtotal;
export const getMembershipFee = ({ accounts }: GlobalState) =>
  accounts.accountOverview.overview.currentPlan.membershipFee;
export const getPlanType = ({ accounts }: GlobalState) => accounts.accountOverview.overview.currentPlan.type;
export const getOrderNumber = ({ accounts }: GlobalState) => accounts.accountOverview.overview.subscriptionId;
export const getNumberOfFreeTrips = ({ accounts }: GlobalState) => accounts.accountOverview.overview.numberOfFreeTrips;
export const getTaxRate = ({ accounts }: GlobalState) => accounts.accountOverview.overview.taxRate;
export const getDeliveryArea = ({ accounts }: GlobalState) => accounts.accountOverview.overview.deliveryAreaIdentifier;
export const getLeases = ({ accounts }: GlobalState) => accounts.accountOverview.overview.leases;
...
export const getTaxAmount = createSelector(
  ...
  (monthlySubtotal: number, membershipFee: number, taxRate: number) =>
    ((monthlySubtotal + membershipFee) * (taxRate * 0.01)).toLocaleString(undefined, {
...
export const getGrandTotal = createSelector(
  ...
  (monthlySubtotal: number, membershipFee: number, taxAmount: string) =>
    (monthlySubtotal + membershipFee + parseFloat(taxAmount)).toLocaleString(undefined, {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2
    })
);

export const getDiscounts = ({ accounts }: GlobalState) => accounts.accountOverview.overview.discounts;
...
export const getStartDate = ({ accounts }: GlobalState) => {
  ...
  return startDate ? formatDate(parseISO(startDate), 'MMMM d, yyyy') : 'Pending';
};

export const getRawStartDate = ({ accounts }: GlobalState) => accounts.accountOverview.overview.startDate;
...
export const hasUpcomingDelivery = createSelector(getRawStartDate, (startDate: string | null) => {
...
export const getMembershipPageFAQsID = createSelector(getPlanType, (planType: PlanType) =>
  planType === 'Member'
...
export const hasSignedLease = createSelector(getLeases, (leases: Lease[]) => {
  const activeLeases = leases.filter((lease) => ['Pending', 'Signed'].includes(lease.status));
