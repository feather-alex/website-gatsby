...
export function* getAccountOverview(): SagaIterator {
  try {
    const response: Subscription = yield call([Request, 'send'], RequestMethod.GET, '/account/overview');
...
export function* handleRedirect(): SagaIterator {
  if (history.location.pathname === '/account') {
    const hasUpcomingDelivery = yield select(accountOverviewSelector.hasUpcomingDelivery);
    const pathname = hasUpcomingDelivery ? '/account/delivery' : '/account/furniture';
