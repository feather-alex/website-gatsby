...
class PaymentSources extends React.Component<Props, State> {
  ...
  renderPaymentSources = () => {
    const { defaultPaymentSource, showBillingActions, sources, isPastDue } =
      this.props;
            ...
            return (
              <div
                ...
                css={css`
                  ...
                  ${isPastDue &&
                  isPrimary &&
                  `color: ${BRAND.ERROR}; margin-left: -5px;`}
                `}
              >
                {isPastDue && isPrimary ? (
                  <ErrorIcon widthAndHeight={16} />
                ) : null}
                ...
                >
                  <Title2
                    isBold={isPrimary}
                  >{`Card ending in ${card.lastFour}`}</Title2>
                ...
                >
                  {showBillingActions && isPrimary && sources.length > 1 && (
                    <Button
                      ...
                      onClick={() =>
                        this.props.removeBillingCardRequest(card.id)
                      }
                    >
                      Remove
                    </Button>
                  ...
                  {showBillingActions && !isPrimary && (
                    <React.Fragment>
                      <Button
                        style={ButtonStyle.TEXT}
                        onClick={() =>
                          this.props.setPrimaryCardRequest(card.id)
                        }
                      >
                        Make Primary
                      </Button>
  ...
  render() {
    const { showBillingActions, showNewCardForm, isFetching, error } =
      this.props;
      ...
      >
        ...
        <div
          css={css`
            ...
            overflow: hidden;
            ${showNewCardForm && showBillingActions
              ? `height: 500px;`
              : `height: 0;`}
          `}
        >
        ...
        {!isFetching && (
          <div
            css={css`
              ...
              margin-top: 15px;
              transition: opacity 0.2s ease-in;
              ${showBillingActions && !showNewCardForm
                ? `opacity: 1;`
                : `opacity: 0;`}
            `}
          >
            <Button
              dataCy="add-card-button"
              onClick={this.props.toggleNewCardForm}
            >
              Add a card
            </Button>
          </div>
