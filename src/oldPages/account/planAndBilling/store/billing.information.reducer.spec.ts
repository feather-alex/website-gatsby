...
import { BillingInformation, BillingSourcesResource } from './billing.information.types';
...
describe('Plan & Billing - Reducers', () => {
  ...
  describe('Getting payment profile', () => {
    ...
    it('should handle successfully fetching payment profile', () => {
      ...
      const action: FluxStandardAction = actions.getPaymentProfileSuccess(paymentProfile);
      const reduced = billingInformation(state, action);
    ...
    it('should handle unsuccessfully fetching payment profile', () => {
      const action: FluxStandardAction = actions.getPaymentProfileFailure(error);
      const reduced = billingInformation(state, action);
  ...
  describe('Add new card', () => {
    it('should handle action to add a payment source info request', () => {
      ...
      const action: FluxStandardAction = actions.addBillingCardRequest(sourceId, isPrimary);
  ...
  describe('Remove billing card', () => {
    ...
    it('should handle failing to remove a billing card', () => {
      const action: FluxStandardAction = actions.removeBillingCardFailure(error);
