...
describe('Billing info - Selectors', () => {
  ...
  it('should return if we are currently fetching', () => {
    const isFetching = selectors.isFetching({ accounts: { billingInformation: state } } as GlobalState);
    expect(isFetching).toEqual(billingInfo.isFetching);
    expect(isFetching).toEqual(false);
  });

  it('should return the current error', () => {
    const error = selectors.getError({ accounts: { billingInformation: state } } as GlobalState);
  ...
  it('should return all sources', () => {
    ...
    const sources = selectors.getSources({ accounts: { billingInformation: state } } as GlobalState);
  ...
  it('should return the billing start date', () => {
    ...
    const startDate = selectors.getBillingStartDate({ accounts: { billingInformation: state } } as GlobalState);
