...
describe('Chnage Password - Selectors', () => {
  ...
  it('should return isFetching', () => {
    const isFetching = selectors.getIsFetching({ accounts: { changePassword: state } } as GlobalState);
    expect(isFetching).toEqual(initialState.isFetching);
  });

  it('should return isPasswordUpdated', () => {
    const isPasswordUpdated = selectors.getIsPasswordUpdated({ accounts: { changePassword: state } } as GlobalState);
    expect(isPasswordUpdated).toEqual(initialState.isPasswordUpdated);
  });

  it('should return error as null', () => {
    const error = selectors.getError({ accounts: { changePassword: state } } as GlobalState);
