...
import { logOut } from "../../../../oldPages/auth/login/store/login.actions";

// Get Account History
export function* getAccountHistoryRequest(): SagaIterator {
  try {
    const pageIncrements: number = yield select(
      accountHistorySelectors.getPerPage
    );
    const lastChargeId: string | undefined = yield select(
      accountHistorySelectors.getLastChargeId
    );

    const queryParams: QueryParam[] = [
      { name: "limit", value: pageIncrements.toString() },
    ];
