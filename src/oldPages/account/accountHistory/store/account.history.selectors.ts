...
export const isFetching = ({ accounts }: GlobalState) =>
  accounts.accountHistory.isFetching;
export const getPaymentsData = ({ accounts }: GlobalState) =>
  accounts.accountHistory.charges;
export const getError = ({ accounts }: GlobalState) =>
  accounts.accountHistory.error;
export const getPerPage = ({ accounts }: GlobalState) =>
  accounts.accountHistory.perPage;
export const getHasMoreAccountHistory = ({ accounts }: GlobalState) =>
  accounts.accountHistory.hasMore;

export const getTotal = ({ accounts }: GlobalState) =>
  accounts.accountHistory.charges.length;
...
export const getPastDue = createSelector(
  ...
  (payments: PaymentDetails[], defaultSource: BillingResource) => {
    return (
      ...
      payments[0].sourceId === defaultSource.id
    );
  }
);
