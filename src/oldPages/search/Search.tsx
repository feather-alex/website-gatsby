...
const SearchInputContainer = styled.div`
  ...
  border-bottom: 1px solid
    ${({ isQueryPresent }: { isQueryPresent: boolean }) =>
      isQueryPresent ? BRAND.PRIMARY_TEXT : SHADES.SHADE_LIGHTER};
`;
  ...
  if (productsError) {
    return (
      <Layout>
        ...
        <ErrorPage
          title={`${productsError.status} ${productsError.error}`}
          content={productsError.message}
        />
      </Layout>
    );
  }
  ...
  if (packagesError) {
    return (
      <Layout>
        <Helmet title={TITLE} description={DESCRIPTION} />
        <ErrorPage
          title={`${packagesError.status} ${packagesError.error}`}
          content={packagesError.message}
        />
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      ...
      <SearchContainer>
        <SearchInputContainer isQueryPresent={!!query}>
          ...
          <SearchButton onClick={handleClick} isDisabled={isDisabled}>
            <SearchIconContainer>
              <SearchIcon
                color={isDisabled ? SHADES.SHADE_LIGHT : BRAND.PRIMARY_TEXT}
              />
            </SearchIconContainer>
      ...
      <ResultsContainer>
        <ResultsHeader>
          {!!keyword &&
            ...
            (!isResults ? (
              ...
              <NumResults>
                We found {productTotal + packageData.length} results for "
                {keyword}"
              </NumResults>
            ))}
        </ResultsHeader>
        ...
        <Results
          areResultsAvailable={isProductResults || isPackageResults}
