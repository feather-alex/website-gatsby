...
describe('Search Page - Reducer', () => {
  ...
  it('Should handle action: GET_SEARCH_PRODUCTS_SUCCESS (with infinite loading)', () => {
    ...
    // TODO: Fix this the next time the file is edited.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const expectedProductsData = state.products.data.concat(payload.pageData as any);
  ...
  it('Should handle action: RESET_SEARCH', () => {
    ...
    expect(reduced.packages.isFetching).toEqual(initialState.packages.isFetching);
  });
});
