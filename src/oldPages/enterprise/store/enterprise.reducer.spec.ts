...
describe('Enterprise - Reducer', () => {
  ...
  it('Should handle action: GET_ENTERPRISE_SUCCESS', () => {
    ...
    expect(reduced.horizontalLockup).toEqual(mockSuccessPayload.horizontalLockup);
    expect(reduced.horizontalLockup2).toEqual(mockSuccessPayload.horizontalLockup2);
    expect(reduced.productShowcase).toEqual(mockSuccessPayload.productShowcase);
    expect(reduced.titleButtonLockup).toEqual(mockSuccessPayload.titleButtonLockup);
    expect(reduced.titledTripleVerticalImageLockup).toEqual(mockSuccessPayload.titledTripleVerticalImageLockup);
    expect(reduced.error).toBeNull();
  });
