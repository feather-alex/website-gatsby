...
import { BillingAddressInfoFields, CustomerInfoFields, DeliveryInfoFields, SSNInfoFields } from './checkoutForms.types';
...
export const validateCustomerInfo = (values: CustomerInfoFields): CustomerInfoErrors => {
...
export const validateSSNInfo = (values: SSNInfoFields) => {
  ...
  if (!validateSSN(values.ssn)) {
    errors.ssn = 'Invalid SSN Format. Please make sure your number is accurate and contains 9 digits';
  }

  if (!validateLegalName(values.legalFirstName)) {
    errors.legalFirstName = 'Legal first name is required and must be less than 250 characters';
  }

  if (!validateLegalName(values.legalLastName)) {
    errors.legalLastName = 'Legal last name is required and must be less than 250 characters';
  }
...
export const validateDeliveryInfo = (values: DeliveryInfoFields): DeliveryInfoErrors => {
...
export const validateBillingAddressInfo = (values: BillingAddressInfoFields): BillingAddressErrors => {
