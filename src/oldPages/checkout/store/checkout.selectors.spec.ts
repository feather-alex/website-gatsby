...
describe('Checkout selectors', () => {
  ...
  it('should get whether we should generate a new stripe token', () => {
    expect(getGenerateNewStripeToken({ checkout: state } as GlobalState)).toEqual(false);
  });

  it('should get whether an order is currently being placed', () => {
    expect(getIsPlacingOrder({ checkout: state } as GlobalState)).toEqual(false);
  });

  it('should get whether a deposit request is currently being submitted', () => {
    expect(getIsSubmittingDeposit({ checkout: state } as GlobalState)).toEqual(false);
  });

  it('should get the current card error', () => {
    expect(getCardError({ checkout: state } as GlobalState)).toEqual({ error: null });
  });
  ...
  it('should get whether an associated credit profile was not found', () => {
    expect(getIsCreditNotFound({ checkout: state } as GlobalState)).toEqual(false);
  });
  ...
  it('should get whether an amount is being fetched', () => {
    expect(getIsFetchingAmount({ checkout: state } as GlobalState)).toEqual(false);
  });
  ...
  it('should get delivery fee', () => {
    ...
    expect(getDeliveryFee({ checkout: state } as GlobalState)).toEqual(deliveryFee);
  });
  ...
  it('should get monthly total', () => {
    ...
    expect(getMonthlyTotal({ checkout: state } as GlobalState)).toEqual(monthlyTotal);
  });
  ...
  it('should get membership fee', () => {
    ...
    expect(getMembershipFee({ checkout: state } as GlobalState)).toEqual(membershipFee);
  });
  ...
  it('should get deposit amount', () => {
    ...
    expect(getDepositAmount({ checkout: state } as GlobalState)).toEqual(depositAmount);
  });
  ...
  it('should get the billing address info', () => {
    ...
    expect(getBillingAddressInfo({ checkout: state } as GlobalState)).toEqual(info);
  });

  it('should get if billing is the same as delivery', () => {
    expect(getIsDeliverySameAsBilling({ checkout: state } as GlobalState)).toEqual(true);
  });
