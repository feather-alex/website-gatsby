...
import createRootReducer, {
  initialState as fullInitialState,
} from "../../../store/reducer";
...
describe("<SSNEntry />", () => {
  ...
  it("renders correct error message for ssn when error comes from client-side validation and SSN has a value", async () => {
    ...
    fireEvent.blur(getByPlaceholderText("Enter your SSN"));
    fireEvent.change(getByPlaceholderText("Enter your SSN"), {
      target: { value: mockSSN },
    });
  ...
  it("renders correct error message for ssn when error comes from client-side validation and SSN is empty", async () => {
    const store = createStore(
      createRootReducer(createMemoryHistory({ keyLength: 0 })),
      fullInitialState
    );
    store.dispatch = jest.fn();
  ...
  it("renders the correct error message for first name when error comes from client-side validation and first name has more than 250 characters", async () => {
    ...
    fireEvent.change(getByPlaceholderText("Enter your legal first name"), {
  ...
  it("renders the correct error message for last name when error comes from client-side validation and last name has a value", async () => {
    ...
    fireEvent.change(getByPlaceholderText("Enter your legal last name"), {
      target: { value: mockLastName },
    });
  ...
  it("updates on change correctly", async () => {
    ...
    await act(async () => {
      await fireEvent.change(getByPlaceholderText("Enter your SSN"), {
        target: { value: mockSSN },
      });
      await fireEvent.change(
        getByPlaceholderText("Enter your legal first name"),
        { target: { value: mockFirstName } }
      );
      await fireEvent.change(
        getByPlaceholderText("Enter your legal last name"),
        { target: { value: mockLastName } }
      );
    });
    expect(mockDispatch).toHaveBeenCalledWith(
      updateSSNInfo({
        ssn: mockSSN,
        legalFirstName: mockFirstName,
        legalLastName: mockLastName,
      })
    );
  });
  ...
  it(`calls the function to open the ssn overlay when the customer doesn't have a SSN`, () => {
    ...
    expect(mockDispatch).toHaveBeenCalledWith(
      toggleOverlay(Overlays.NoSSNOverlay, true)
    );
  });
});
