...
import { CheckoutNextStepButtonContainer, CheckoutPageForm, FormHeader } from './CheckoutStyledComponents';
import { MembershipStateDisplayName } from '../../../app/store/plan/plan.types';
import { customerInfoPayloadMapping, stepViewedPayloadMapping } from '../../../analytics/checkout/payload-mappings';
...
const CheckoutCustomerInfo = ({ backToCart }: Props) => {
  ...
  useMount(() => {
    ...
    dispatch(resetPromo());
    Analytics.trackEvent(CHECKOUT.STEP_VIEWED, stepViewedPayloadMapping({ step: CheckoutStep.CustomerInfo, cartUuid }));
  });
  ...
  const handleIntendedUseOtherChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setIntendedUseOther({
  ...
  const handleNextStep = (data: CustomerInfoFields) => {
    ...
    dispatch(checkoutStepCompleted({ step: CheckoutStateStep.CustomerInfo, data: { ...data, persona } }));
  ...
  const { handleChange, handleBlur, handleSubmit, values, errors, touched } = useFormik<CustomerInfoFields>({
    initialValues: {
  ...
  const areFieldsEmpty = values.firstName.length === 0 && values.lastName.length === 0 && values.email.length === 0;
