...
const popover = (
  <Popover className="options-popover" id="popover-positioned-bottom" style={{ width: '400px' }} placement="bottom">
    ...
    <p className="futura">
      Soft inquiries (also known as “soft pulls”) typically occur when a person or company checks your credit as part of
      a verification process. This may occur, for example, when a credit card issuer checks your credit to see if you
      qualify for certain credit card offers. Your employer might also run a soft inquiry before hiring you. We use a
      VantageScore (versus a FICO score) to better to take into account students and people without a long credit
      history.
    </p>
  </Popover>
);
...
const SSNEntry = () => {
  ...
  const { handleBlur, values, errors, touched, handleChange } = useFormik<SSNInfoFields>({
    ...
    <Fragment>
      <Title2 dataCy="ssn-error">
        <span
          ...
          `}
        >
          Unfortunately, we were unable to find your credit score using your provided name and billing address.
        </span>
        <br />
        <br />
        Please submit your Social Security Number and verify your legal name, and we’ll run the soft check again.
      </Title2>
      <SectionPadding>
        <HeaderPadding>
      ...
      <SectionPadding>
        ...
        >
          <CheckoutInputFieldFormik
            ...
            handleChange={(e: ChangeEvent<HTMLInputElement>) => {
              dispatch(updateSSNInfo({ ...values, legalFirstName: e.target.value }));
              handleChange(e);
            }}
            handleBlur={handleBlur}
          />
        ...
        <CheckoutInputFieldFormik
          ...
          handleChange={(e: ChangeEvent<HTMLInputElement>) => {
            dispatch(updateSSNInfo({ ...values, legalLastName: e.target.value }));
            handleChange(e);
          }}
          handleBlur={handleBlur}
        />
      ...
      >
        <div
          ...
          `}
        >
          <Title2>Placing your order and running the soft check will not affect your credit score. </Title2>
          <OverlayTrigger trigger="click" overlay={popover} placement="bottom" rootClose={true}>
            <TooltipIcon
              className="icon"
              css={css`
                margin-left: 5px;
              ...
              `}
              onClick={() =>
                Analytics.trackEvent(CHECKOUT.CLICK_TOOLTIP, checkoutActionsCartUuidPayloadMapping({ cartUuid }))
              }
            />
          </OverlayTrigger>
        </div>
