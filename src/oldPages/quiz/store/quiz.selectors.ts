...
export const getLocationPathBeforeQuizOpened = ({ quiz }: GlobalState) =>
  quiz.locationPathBeforeQuizOpened;
...
export const getCurrentStepIndex = ({ quiz }: GlobalState) =>
...
export const getNumberOfBedrooms = ({ quiz }: GlobalState) =>
  quiz.numberOfBedrooms;
export const getCurrentBedroomIndex = ({ quiz }: GlobalState) =>
  quiz.currentBedroomIndex;
...
export const getSanitizedRooms = createSelector(getRooms, (originalRooms) => {
  ...
  if (
    ...
    (rooms.bedrooms.length === 1 && rooms.bedrooms[0].bedSize === "")
  ) {
