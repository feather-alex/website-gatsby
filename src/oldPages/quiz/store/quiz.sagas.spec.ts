...
import { getQuizAnswers, getLocationPathBeforeQuizOpened } from './quiz.selectors';
...
import { QuizAnswers, FurnitureReason, BudgetTier, DeliveryAreaChoice } from './quiz.types';
...
describe('Quiz Sagas', () => {
  ...
  it('should handle quiz submission', () => {
    ...
    return expectSaga(handleQuizSubmission, action)
      .provide([
        [matchers.call(Analytics.trackUser, { properties: { name, email } }), noop],
        [matchers.call(Analytics.trackEvent, QUIZ.SUBMITTED, quizFinalStepPayloadMapping(action.payload)), noop],
        [matchers.select(getQuizAnswers), mockQuizAnswers]
      ])
      .call(Analytics.trackUser, { properties: { name, email } })
      .call(Analytics.trackEvent, QUIZ.SUBMITTED, quizFinalStepPayloadMapping(action.payload))
  ...
  it('should handle location change to capture the URL before the quiz was opened', () => {
    ...
    return expectSaga(handleLocationChange, action)
      .provide([[matchers.select(getLocationPathBeforeQuizOpened), '/how-it-works']])
