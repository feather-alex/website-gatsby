...
import {
  Categories,
  ProductCategory,
} from "../../app/store/entities/entities.types";
...
export const getVariantLink = (
  product: ProductForListing,
  variant: VariantDetails
): string => {
  const productLinkPrefix = `/products/${product.identifier}`;
  const productLinkSuffix =
    product.variants.length > 1 ? `?variant=${variant.identifier}` : "";
  return productLinkPrefix + productLinkSuffix;
};

export const getIsNoVariantsInAnyArea = (
  product: ProductForListing,
  location: string
) =>
  location === "all" &&
  !product.variants.some((variant) =>
    variant.availability.some((availability) => availability.isEnabled)
  );

export const getIsNoVariantsInSelectedArea = (
  product: ProductForListing,
  location: string
) =>
  location !== "all" &&
  !product.variants.some((variant) =>
    variant.availability.some(
      (availability) =>
        availability.isEnabled && availability.deliveryArea === location
    )
  );

export const getShouldShowProduct = (
  product: ProductForListing,
  deliveryArea: DeliveryAreaIdentifier
) => {
  return (
    !getIsNoVariantsInSelectedArea(product, deliveryArea) ||
    !getIsNoVariantsInAnyArea(product, deliveryArea)
  );
};

export const isThereAVariantPriceDifference = <
  T extends { retailPrice: number }
>(
  variants: T[]
) => {
  return variants.some(
    (variant) => variant.retailPrice !== variants[0].retailPrice
  );
};
...
export const getHelmetData = (
  ...
  let image =
    "https://img.livefeather.com/pages-new/Homepage/two.jpg?auto=compress&sat=35&q=65&sharp=25&w=300&dpr=1.7";
  const description =
  ...
  if (categories && selectedCategory !== "") {
    const category = categories.products.find(
      (x: ProductCategory) => x.identifier === selectedCategory
    );
    ...
    if (
      ...
      selectedSubclassFilters.length === 1
    ) {
...
export const groupSubclasses = (
  subclasses: IdentifierAndName[]
): IdentifierAndName[] =>
  Array.from(
    new Set(
      subclasses.map(
        (subclass) => subclassMappings[subclass.identifier] || subclass
      )
    )
  );
...
export const sortSubclasses = (subclasses: IdentifierAndName[]) =>
  subclasses.sort(
    (a, b) => subclassRankings[a.identifier] - subclassRankings[b.identifier]
  );
...
export const formatProductListResponse = (
  ...
  const meta = {
    ...
    availableFilters: {
      ...
      subclasses: sortSubclasses(
        groupSubclasses(response.availableFilters.subclasses)
      ),
    },
  };
