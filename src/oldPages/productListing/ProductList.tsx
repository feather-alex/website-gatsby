...
import {
  ...
  isThereAVariantPriceDifference,
  getVariantLink,
...
const ProductList = ({
  ...
  const renderProducts = React.useCallback((): (
    ...
    return products.map((product, index) => {
      ...
      const isWaypoint =
        index === Math.ceil(productsLength / 2) || index === productsLength - 1;
      ...
      const { elements } = product.variants.reduce(
        (
          acc: { variants: VariantDetails[]; elements: JSX.Element[] },
          variant,
          idx: number
        ) => {
          ...
          if (
            acc.variants.filter(
              (accVariant) =>
                accVariant.listingImage.desktop === variant.listingImage.desktop
            ).length
          ) {
            return acc;
          }
          ...
          const featherPrice =
            membershipState === MembershipState.NON_MEMBER
              ? variant.rentalPrices[3]
              : variant.rentalPrices[12];
          ...
          const shouldShowFromPrice =
            ...
            variant.options.some((option) =>
              ["Twin", "Full", "Full/Queen", "Queen", "King"].includes(
                option.valueName
              )
            );
            ...
            elements: [
              ...
              <ItemCard
                ...
                intersectionCallback={
                  isWaypoint && idx === 0 ? loadMoreProducts : undefined
                }
              />,
            ],
          };
