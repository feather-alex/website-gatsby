...
type Props = StateProps &
  DispatchProps &
  RouteComponentProps<{ productIdentifier?: string }>;
  ...
  getProductList = (offset = 0) => {
    ...
    const newCategoriesSubclassesMap: { [name: string]: string[] } = {
      ...
      chairs: [
        ...
        "non-upholstered-occasional-chair",
        "desk-chair",
        "dining-chair",
      ],
    ...
    const isInNewCategories = category
      ? Boolean(newCategoriesSubclassesMap[category])
      : false;
    ...
    Object.entries(subclassMappings).forEach(([subclass, mapping]) => {
      if (filterMap[FilterType.SUBCLASS].includes(mapping.identifier)) {
        filterMap[FilterType.SUBCLASS] =
          filterMap[FilterType.SUBCLASS].concat(subclass);
      }
    });
    ...
    this.props.getProductList({
      body: {
        ...
        sort:
          (filterMap[FilterType.SORT_BY][0] as "price" | "title" | undefined) ||
          null,
        order:
          (filterMap[FilterType.ORDER][0] as "a" | "d" | undefined) || null,
        categories: !isInNewCategories && category ? [category] : [],
        classes: [],
        subclasses:
          isInNewCategories && category
            ? newCategoriesSubclassesMap[category]
            : [],
        filter: {
    ...
    return (
      <Layout>
        ...
        {error ? (
          ...
          <Fragment>
            <ProductHeader
              categories={productEntities ? productEntities.categories : null}
            />
            <div
              css={css`
                  ...
                  {isFetching && !isResults ? (
                    ...
                    >
                      <Loading
                        message={
                          <Fragment>
                            Beautiful furniture is just <br /> around the
                            corner!
                          </Fragment>
                        }
                      />
