...
const ProductFilterOption = ({ option, filterType }: Props) => {
  ...
  const handleToggle = React.useCallback(
    (identifier: string) => () => {
      const newQuery = createQuery({ identifier, filterType, locationSearch: location.search });
  ...
  const isChecked = Boolean(filters.some((filter: string) => filter === option.identifier));
  return (
    <div
      css={css`
        padding-left: 2px;
    ...
    >
      <SelectInput inputType={InputType.Checkbox} onChange={handleToggle(option.identifier)} isChecked={isChecked}>
        <span
          css={css`
