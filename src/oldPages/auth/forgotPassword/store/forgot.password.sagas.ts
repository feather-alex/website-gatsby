...
import { RESET_PASSWORD_REQUEST, resetPasswordSuccess, resetPasswordFailure } from './forgot.password.actions';

export function* handleResetPassword(action: FluxStandardAction): SagaIterator {
  try {
    yield call([Request, 'send'], RequestMethod.POST, '/auth/reset-password', undefined, action.payload.email, true);
