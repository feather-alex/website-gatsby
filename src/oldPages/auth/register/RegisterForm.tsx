...
const RegisterForm = (
  props: Props & InjectedFormProps<RegisterFormData, Props>
) => {
  const {
    isFetching,
    handleSubmit,
    ...
    <RegistrationForm onSubmit={handleSubmit(onSubmit)}>
      {initialValues.email ? (
        <React.Fragment>
          ...
          <EmailParagraph dataCy="email-value">
            {initialValues.email}
          </EmailParagraph>
        </React.Fragment>
      ) : (
      ...
      />
      {setupAccountError && (
        <Title2 color={BRAND.ERROR}>
          Something went wrong when trying to register. Please try again.
        </Title2>
      )}
      {initialValues.email ? (
