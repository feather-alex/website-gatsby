  ...
  describe('send', () => {
    ...
    describe('success', () => {
      ...
      it('sends a GET request', async () => {
        ...
        expect(window.fetch).toHaveBeenCalledWith('/test', { headers: new Headers(), method: 'GET' });
      });

      it('sends a POST request', async () => {
        const response = await Request.send(RequestMethod.POST, '/test', undefined, 'test');
        expect(response).toEqual(JSON.parse(mockBody));
      ...
      it('sends a PUT request', async () => {
        const response = await Request.send(RequestMethod.PUT, '/test', undefined, 'test');
        expect(response).toEqual(JSON.parse(mockBody));
        expect(window.fetch).toHaveBeenCalledWith(
          '/test',
          expect.objectContaining({
      ...
      it('sends a DELETE request', async () => {
        const response = await Request.send(RequestMethod.DELETE, '/test', undefined, 'test');
        expect(response).toEqual(JSON.parse(mockBody));
        expect(window.fetch).toHaveBeenCalledWith(
          '/test',
          expect.objectContaining({
    ...
    describe('failure', () => {
      ...
      it('throws an APIError on status 400', async () => {
        ...
        await expect(Request.send(RequestMethod.POST, '/test', undefined, 'test')).rejects.toThrowError(
          expect.objectContaining({
            status: 400,
            error: 'Bad Request'
          })
      ...
      it('throws a StripeError on status 500', async () => {
        const mockBody = { type: 'StripeCardError', message: 'stripe error', code: 1234 };
        mockFetch(mockBody, 500);
      ...
      it('throws a Too many attempts error on status 500', async () => {
        const mockBody = {
          error: JSON.stringify({ error: 'too_many_attempts', error_description: 'test description' })
        };
        mockFetch(mockBody, 500);
