...
import { APIError, isAPIError, StripeError, isStripeError, isTooManyAttemptsError } from './error';
...
export default class Request {
  ...
  private static handleResponseData(response: Response, isPlainText: boolean) {
    ...
    switch (response.status) {
      ...
      case 400:
        if (!response.json) {
          throw new APIError('The request data is invalid.', 'Bad Request', response.status);
        }
        return response.json().then((error) => {
          throw new APIError('The request data is invalid.', 'Bad Request', response.status, error);
        });

      // UNAUTHORIZED
      case 401:
        throw new APIError('Access is denied due to invalid credentials.', 'Unauthorized', response.status);

      // FORBIDDEN
      case 403:
        throw new APIError("You don't have enough permission to view this.", 'Forbidden', response.status);
      ...
      case 500: {
        ...
        return response.json().then((jsonObject) => {
          if (isStripeError(jsonObject)) {
            throw new StripeError(jsonObject.message, response.status, jsonObject.code);
          } else if (isTooManyAttemptsError(jsonObject)) {
            const error = JSON.parse(jsonObject.error);
            throw new APIError(error.error_description, error.error, response.status);
          } else {
            throw new APIError(
              'There is something wrong with the server at the moment.',
              'Internal Server Error',
      ...
      default:
        ...
        return response.json().then((error) => {
          ...
          throw new APIError(error.message || error.detail, error.error, error.status);
        });
    }
  }
}
