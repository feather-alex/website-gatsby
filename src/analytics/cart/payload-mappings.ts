import { CartItem, CartPkgItem } from "../../oldPages/cart/store/cart.types";
...
export const cartViewedPayloadMapping = ({
  ...
  products: cartItems.map((cartItem) => ({
    ...
    category: cartItem.categories
      ? cartItem.categories[0].identifier
      : "unknown",
    name: cartItem.title,
    ...
    image_url: `${
      cartItem.image.desktop || cartItem.image.mobile
    }?auto=compress,format`,
  })),
  ...
  const cartItemsSelected = cartItems.map((item) => ({
    ...
    product_category_identifiers:
      item.categories && item.categories.length
        ? item.categories.map((cat: IdName) => cat.identifier)
        : null,
