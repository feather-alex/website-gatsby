import { ProductForListing, ProductListRequest, VariantDetails } from '../../types/Product';
...
export const productClickedPayloadMapping = ({
  ...
  image_url: `${variant.listingImage.desktop || variant.listingImage.mobile}?auto=compress,format`
});

const convertProducts = (category: string) => (
...
export const filterListPayloadMapping = ({
  ...
  filters: [
    ...
    ...body.filter.classes.map((className) => ({ type: 'class', value: className })),
    ...body.filter.subclasses.map((subclass) => ({ type: 'subclass', value: subclass }))
  ],
  sorts: [
    {
      type: body.sort,
...
export const removeOrAddFilterPayloadMapping = ({ filterClicked }: { filterClicked: string }) => ({
  filter_clicked: filterClicked
});

export const openOrCloseFiltersPayloadMapping = ({ filters }: { filters: string }) => ({
